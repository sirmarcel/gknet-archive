...
data = xr.open_dataset("../../../gknet-results/data/vdos/result_300.nc")
...
for t in [1, 2]:
    ...
    plot_peaks(
        ax, data["aims"].mean(dim="trajectory"), color=red, prominence=prominence
    )
    plot_vdos(ax, data["aims"], label="FHI-aims", color=red)

    # colors = cycle(tol_vibrant[1:])
    for i, cutoff in reversed(list(enumerate(np.arange(4.5, 6.51, 0.5)))):
        name = f"cu{cutoff:.1f}_n{t}"
        plot_vdos(ax, data[name], color=black, offset=0.8 * (i + 1), label=rc(cutoff))
    ...
    savefig(fig, f"../../gknet-paper/img/vdos_t{t}.pdf")
