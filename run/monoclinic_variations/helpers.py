...
def write_main(folder, temperature, size):
    ...
    if temperature == 300:
        shutil.copy(
            f"../oneoff_compare_cells_1/cells/{temperature}_new_geometry.in.primitive",
            folder / "nvt/geometry.in.primitive",
        )
        shutil.copy(
            f"../oneoff_compare_cells_1/cells/{temperature}_new_geometry.in.primitive.supercell_{size}",
            folder / "nvt/geometry.in.supercell",
        )
        shutil.copy(
            f"../oneoff_compare_cells_1/cells/{temperature}_new_geometry.in.primitive.supercell_{size}.0010K",
            folder / "nvt/geometry.in",
        )
    else:
        shutil.copy(
            f"../monoclinic_exp_real_prod_1/cells/{temperature}_geometry.in.primitive",
            folder / "nvt/geometry.in.primitive",
        )
        shutil.copy(
            f"../monoclinic_exp_real_prod_1/cells/{temperature}_geometry.in.primitive.supercell_{size}",
            folder / "nvt/geometry.in.supercell",
        )
        shutil.copy(
            f"../monoclinic_exp_real_prod_1/cells/{temperature}_geometry.in.primitive.supercell_{size}.0010K",
            folder / "nvt/geometry.in",
        )
