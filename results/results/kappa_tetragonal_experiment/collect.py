...
prod_folder = basedir / "cu_5.0_e_inpt_4387/train_n2_s1/tetragonal_exp_real_prod_1/"
test_folder = basedir / "cu_5.0_e_inpt_4387/train_n2_s1/variations_tex_2/"

a = None
eps = 0
test_temps = [1400]
prod_temps = [1500, 1650, 1800]
all_temps = list(sorted(test_temps + prod_temps))
...
def get_test_dataset(temp, freq=3.00):
    return gk.combine(
        load.ensemble(
            test_folder / f"{temp}/n_{n_atoms}",
...
def get_data(freq):
    ...
    for i, temp in enumerate(all_temps):
        ...
        if temp in test_temps:
            data[i] = gk.get_kappa(get_test_dataset(temp, freq=freq), a=a, eps=eps)
        else:
